...
import { useDeleteMehfilDraft, useMehfilDrafts } from "@/lib/mehfilDrafts";
...
type Props = {
  ...
  onLoad: (draft: MehfilCreateDraft, draftId: string) => void;
  onSaveCurrent: () => void;
  saving?: boolean;
};

export default function MehfilDraftsSheet({ open, onOpenChange, onLoad, onSaveCurrent, saving }: Props) {
  const { data: drafts = [], isLoading } = useMehfilDrafts();
  const del = useDeleteMehfilDraft();
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        ...
        style={{ background: "#0A0806", color: FEED_TEXT, borderColor: FEED_BORDER }}
      >
        <SheetHeader>
          <SheetTitle className="font-['Playfair_Display'] text-left" style={{ color: FEED_TEXT }}>
            Mehfil drafts
          </SheetTitle>
        </SheetHeader>
        <button
          ...
          {drafts.map((d) => (
            ...
            >
              <button
                ...
                onClick={() => {
                  onLoad({ ...d.payload, serverDraftId: d.id }, d.id);
                  onOpenChange(false);
                }}
              >
                ...
                <div className="text-[10px] mt-0.5 capitalize" style={{ color: FEED_MUTED }}>
                  {d.payload.category ?? "mehfil"} · {new Date(d.updatedAt).toLocaleDateString()}
                </div>
              </button>
