...
import { useUserByHandle, useFollowing, useFollow, getCurrentUserId } from "@/lib/store";
...
export default function Following() {
  ...
  const { data: list = [] } = useFollowing(user?.id ?? "");
  const { data: myFollowing = [] } = useFollowing(me);
  const follow = useFollow();
  useTitle(user ? `${user.displayName} · Following` : "Following");
  ...
  return (
    <div className="min-h-screen w-full bg-background">
      <div className="px-5 py-3 flex items-center gap-3 sticky top-0 bg-background/85 backdrop-blur-md z-20">
        ...
        <div>
          <div className="font-['Playfair_Display'] text-[18px] leading-tight">Following</div>
      ...
      <div className="px-5 py-4 space-y-2">
        ...
        {list.length === 0 && <div className="text-center text-muted-foreground text-[13px] py-10">Not following anyone yet.</div>}
      </div>
    </div>
  );
}
